...
const basePath = (process.env.PAGES_BASE_PATH ?? "/build-canvas").replace(
  /\/$/,
  "",
);
...
if (!html.includes(`${basePath}/assets/`)) {
  throw new Error("The rendered page does not reference the Pages base path.");
}
...
console.log(`Portfolio artifact ready: ${outputDir}`);
