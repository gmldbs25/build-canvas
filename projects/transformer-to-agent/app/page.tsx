...
const PAGES: PageDefinition[] = [
  ...
  },
  {
    ...
    title: "코드는 실제로 바뀌었다",
    beats: [
      "배포 직후의 500 오류와, 개발자가 제공한 로그·수정 요청이 출발점입니다.",
      "Agent Runtime은 조사·수정·테스트를 연결하고, 그 결과를 다음 판단으로 돌려보냅니다.",
    ],
  },
  {
    id: "core-question",
    ...
    beats: [
      "언어 모델은 다음 출력을 예측하지만, 파일과 터미널을 직접 실행하지는 않습니다.",
      "Agent System은 LLM 바깥의 Context·Tool·Runtime·Permission·Feedback를 연결합니다.",
    ],
  },
  {
    ...
    beats: [
      "분류 모델은 정해진 Class 후보를 점수화하고 가장 알맞은 결과를 고릅니다.",
      "언어 모델도 현재 Context에서 다음 Token 후보를 점수화합니다. 공통점은 후보 점수화입니다.",
    ],
  },
  {
    id: "next-token",
    ...
    beats: [
      "현재 Context를 바탕으로 Model은 Vocabulary 전체의 다음 Token 후보를 점수화합니다.",
      "선택된 Token이 Context에 더해지고, 같은 과정이 반복되어 문장이 생성됩니다.",
    ],
  },
  {
    id: "appendix",
...
const ARTICLE_BEAT_COPY: Record<PageId, string[]> = {
  ...
  prologue: [
    "배포 직후 사용자 조회 API의 5xx 비율이 상승했고, 개발자는 production-error.log와 함께 원인 분석·수정·테스트를 요청합니다. Agent가 볼 수 있는 근거는 사용자 요청, 제공된 로그, 허용된 Repository와 Tool뿐입니다.",
    "Agent Runtime은 로그 읽기, 코드 검색, 파일 열기, 테스트 실행, 패치 적용을 반복합니다. 각 결과는 다음 판단의 Context로 돌아갑니다. 실패 재현과 Patch, Test 통과는 로컬 검증이며 실제 재배포 확인은 아직 남아 있습니다.",
  ],
  "core-question": [
  ...
  "next-token": [
    "문자열은 Tokenizer를 거쳐 Token ID와 Embedding으로 바뀌고, Transformer는 Attention으로 현재 Context의 관계를 반영합니다. Model의 마지막 계산은 Vocabulary 각 후보의 Logit과 확률 분포를 만듭니다.",
    "생성 시 하나의 Token이 선택되어 출력 끝에 붙고, 추가된 Token은 다음 예측의 Context가 됩니다. 이 Autoregressive Loop가 반복되면서 한 번의 예측은 문장·코드·구조화된 Tool Request처럼 길이가 있는 출력으로 자랍니다.",
  ],
  appendix: [
...
function PrologueScene({ beat, animate }: { beat: number; animate: boolean }) {
  return <div className={`prologue-scene beat-${beat} ${animate ? "is-animating" : "is-complete"}`}>
    ...
    <section className="developer-request is-visible"><div className="request-avatar"><MessageSquareText /></div><div><span className="mono">DEVELOPER → CODING AGENT</span><p>오늘 배포 이후 일부 사용자 조회 요청이 500으로 실패해.<br />로그를 확인해서 원인을 찾고, 수정한 뒤 테스트해줘.</p><span className="request-attachment"><FileText /> production-error.log · provided context</span></div></section>
    <section className={`agent-trace ${beat >= 1 ? "is-visible" : ""}`} aria-label="Coding Agent의 조사와 수정 흐름"><div className="trace-heading"><span className="mono">LOCAL AGENT LOOP · PREVIEW</span><small>각 결과가 다음 판단의 Context로 돌아감</small></div><div className="trace-steps">{TRACE_STEPS.map(({ icon: Icon, label, sub }, index) => <div className={`trace-step ${beat >= 1 ? "is-done" : ""}`} style={{ "--i": index } as CSSProperties} key={label}><span><Icon /></span><strong>{label}</strong><small>{sub}</small>{index < TRACE_STEPS.length - 1 && <i className="trace-link" />}</div>)}</div></section>
    <div className={`verification-strip ${beat >= 1 ? "is-visible" : ""}`}><span><Check />REPRODUCED</span><span><Check />PATCHED</span><span><Check />VERIFIED LOCALLY</span><span className="is-pending"><Circle />PRODUCTION CONFIRMATION</span></div>
    <p className={`prologue-question ${beat >= 1 ? "is-visible" : ""}`}>다음 Token을 예측하는 Model이 어떻게 Repository를 바꿨을까?</p>
  </div>;
}
...
function CoreQuestionScene({ beat, animate }: { beat: number; animate: boolean }) {
  return <div className={`core-question-scene beat-${beat} ${animate ? "is-animating" : "is-complete"}`}>
    <div className={`agent-system-frame ${beat >= 1 ? "is-visible" : ""}`}><span className="mono">AGENT SYSTEM</span><div className="system-parts">{SYSTEM_PARTS.map((part) => <i key={part}>{part}</i>)}</div></div>
    ...
    <section className="environment-card"><div className="environment-heading"><span className="mono">EXECUTION ENVIRONMENT</span><small>{beat >= 1 ? "Runtime이 연결하고 통제" : "Model의 직접 접근 없음"}</small></div><div className="direct-actions">{DIRECT_ACTIONS.map(({ icon: Icon, label }, index) => <div key={label} style={{ "--i": index } as CSSProperties}><Icon /><span>{label}</span>{beat >= 1 ? <ShieldCheck className="allowed" /> : <X />}</div>)}</div></section>
    <div className={`handoff-request ${beat >= 1 ? "is-visible" : ""}`}><code>{`{ name: "read_file", input: { path } }`}</code><ChevronRight /></div>
    <p className="boundary-caption is-visible">{beat >= 1 ? "Model의 요청을 Runtime이 검증하고 실행한다" : "좋은 문장을 만드는 것 ≠ 환경에서 행동하는 것"}</p>
  </div>;
}
...
export default function Home() {
  ...
  const [travelDirection, setTravelDirection] = useState<TravelDirection>("forward");
  ...
  const commitState = (nextMode: Mode, nextPage: number, nextBeat: number, push = true, animate = true, direction: TravelDirection = "forward") => {
    suppressAnimationRef.current = !animate; setTravelDirection(direction); setMode(nextMode); setPageIndex(nextPage); setBeatIndex(nextBeat); writeUrl(nextMode, nextPage, nextBeat, push);
  };
  ...
  useEffect(() => {
    ...
    const readUrl = () => {
      ...
      suppressAnimationRef.current = true; setTravelDirection("forward"); setMode(nextMode); setPageIndex(nextPage); setBeatIndex(nextBeat);
    };
  ...
  useEffect(() => {
    ...
    setIsAnimating(true);
    const duration = currentPage.id === "opening" ? 760 : 620;
  ...
  const previous = () => {
    if (modeTransition !== "idle") return;
    if (beatIndex > 0) return commitState(mode, pageIndex, beatIndex - 1, true, false, "backward");
    if (pageIndex > 0) commitState(mode, pageIndex - 1, PAGES[pageIndex - 1].beats.length - 1, true, false, "backward");
  };
  ...
  const changeMode = (nextMode: Mode) => {
    if (nextMode === mode || modeTransition !== "idle") return;
    if (timerRef.current) clearTimeout(timerRef.current);
    setIsAnimating(false);
    if (reducedMotion) { commitState(nextMode, pageIndex, beatIndex, true, false); return; }
    setModeTransition("out");
    modeTimerRef.current = setTimeout(() => {
      commitState(nextMode, pageIndex, beatIndex, true, false);
      setModeTransition("in");
      modeTimerRef.current = setTimeout(() => setModeTransition("idle"), 520);
    }, 180);
  };
  const replay = () => { suppressAnimationRef.current = false; setAnimationKey((key) => key + 1); };
  const isAtStart = pageIndex === 0 && beatIndex === 0;
  const isAtEnd = pageIndex === PAGES.length - 1 && beatIndex === currentPage.beats.length - 1;

  return <main className={`site-shell mode-${mode} mode-transition-${modeTransition}`}>
    ...
    <section className="experience-shell" id="main-content" aria-live="polite"><TemplateHeader page={currentPage} index={pageIndex} mode={mode} /><div className="experience-grid"><div className="visual-column"><div className={`experience-stage stage-enter-${travelDirection} ${currentPage.id === "appendix" ? "appendix-stage" : ""}`} key={`${pageIndex}-${beatIndex}-${animationKey}`}><SceneForPage pageIndex={pageIndex} beat={beatIndex} animate={currentPage.id !== "appendix" && mode === "presentation" && isAnimating && !reducedMotion} /></div>{currentPage.id !== "appendix" && <div className="beat-caption" aria-hidden={mode === "article"}><span className="mono">STEP {beatIndex + 1} / {currentPage.beats.length}</span><p>{currentPage.beats[beatIndex]}</p><Button variant="ghost" size="sm" onClick={replay} tabIndex={mode === "article" ? -1 : 0} aria-label="현재 장면 다시 재생"><RotateCcw /> 다시 재생</Button></div>}</div>
      {currentPage.id === "appendix" ? <AppendixArticle hidden={mode === "presentation"} /> : <article className="article-copy-panel" aria-label="현재 장면 해설" aria-hidden={mode === "presentation"}><span className="mono article-label">ARTICLE NOTE · STEP {beatIndex + 1}/{currentPage.beats.length}</span><p className="article-lead">{ARTICLE_COPY[currentPage.id].lead}</p><div className="article-beat-summary"><span className="mono">현재 장면</span><strong>{currentPage.beats[beatIndex]}</strong><p>{ARTICLE_BEAT_COPY[currentPage.id][beatIndex]}</p></div><aside>{ARTICLE_COPY[currentPage.id].note}</aside><SourceLinks sources={ARTICLE_COPY[currentPage.id].sources} />{currentPage.beats.length > 1 && <div className="article-beat-index" aria-label="단계 진행 상태">{currentPage.beats.map((_, index) => <i className={index <= beatIndex ? "is-reached" : ""} key={index} />)}</div>}</article>}
    </div></section>
    <nav className="page-navigation" aria-label="Page와 단계 이동"><div className="progress-copy"><span className="mono">PAGE {String(pageIndex + 1).padStart(2, "0")} / {String(PAGES.length).padStart(2, "0")}</span>{currentPage.beats.length > 1 && <span className="mono">STEP {beatIndex + 1} / {currentPage.beats.length}</span>}</div><div className="progress-line" aria-label={`전체 진행도 ${completedBeats}/${totalBeats}`}><span style={{ width: `${(completedBeats / totalBeats) * 100}%` }} /></div><Button variant="ghost" size="sm" className="navigation-button" onClick={previous} disabled={isAtStart || modeTransition !== "idle"} aria-label="이전 단계 또는 Page"><ArrowLeft /><span>이전</span></Button><Button variant="ghost" size="sm" className="navigation-button" onClick={next} disabled={isAtEnd || modeTransition !== "idle"} aria-label="다음 단계 또는 Page"><span>다음</span><ArrowRight /></Button></nav>
  </main>;
}
