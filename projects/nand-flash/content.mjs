...
export const scenes = [
  ...
  scene('read', '05 · Read', '전자를 세지 않고, 전류를 봅니다', 'Vref는 읽을 때 Cell에 가하는 기준 전압입니다.<br>이 전압에서 전류가 흐르는지 보고 bit를 판정합니다.', 'cell', '읽기는 가능하지만, 자유롭게 덮어쓸 수도 있을까요?', {layout: 'lab', length: 1.5, hold: .7}),
  ...
  scene('address', '06 · 물리에서 Software로', '같은 주소, 다른 자리', 'LBA는 컴퓨터가 저장장치에 요청하는 논리 주소입니다.<br>PPA는 NAND 안에 데이터가 있는 실제 Page 주소입니다.', 'mapping', '주소를 연결해 두면, 데이터가 옮겨져도 찾을 수 있습니다.', {layout: 'bench', length: 1.15}),
  ...
  scene('gc', '06 · 공간 문제와 GC', '이 공간을 다시 쓰려면', 'Invalid가 쌓이면 새로 쓸 공간이 줄어듭니다.<br>GC는 필요한 데이터를 옮기고 Block을 지워 공간을 되찾습니다.', 'mapping', '공간은 되찾았습니다. 지우기를 계속 반복해도 괜찮을까요?', {layout: 'bench', length: 1.8, hold: .75}),
  scene('wear', '07 · 마모 문제와 Wear Leveling', '한곳만 닳지 않도록', 'Program과 Erase의 반복은 Cell을 마모시킵니다.<br>Wear Leveling은 특정 Block만 닳지 않게 사용을 나눕니다.', 'wear', '사용량을 나눠도 기억의 상태는 조금씩 흔들릴 수 있습니다.', {layout: 'lab', length: 1.35}),
  scene('reliability', '08 · 신뢰성 문제의 시작', '기억의 경계가 흔들릴 때', '시간이 지나거나 읽기가 반복되면 Vth가 흔들릴 수 있습니다.<br>상태가 읽기 경계를 넘으면 다른 bit로 판정됩니다.', 'reliability', '조금 틀리게 읽은 데이터를 어떻게 되찾을까요?', {layout: 'lab', length: 1.35}),
  scene('ecc', '08 · 읽기 오류와 ECC / Read Retry', '여분의 bit가 기억을 지킵니다', 'ECC는 여분의 검사 정보로 읽기 오류를 찾고 복구하는 기술입니다.<br>Read Retry는 기준 전압을 바꿔 같은 Cell을 다시 읽는 과정입니다.', 'ecc', '이제 처음의 파일 저장으로 돌아갑니다.', {layout: 'lab', length: 1.8, hold: .75}),
...
// The short story remains intact; these sentences supply the missing first-time context.
export const sceneLearning = {
  question: ['사진, 코드, 게임도 결국 저장해야 할 데이터입니다. 여기서는 문서 하나가 저장과 정상 종료를 거쳐 남는 과정을 따라갑니다.', '문서를 저장한 뒤, 같은 버튼으로 종료하고 다시 켜 보세요.'],
  storage: ['RAM은 전원이 필요한 작업 공간입니다. SSD는 파일을 오래 보관하는 저장장치입니다. 이 탐험은 SSD가 있는 컴퓨터를 기준으로 합니다.', '기판 안에서 기억.txt가 남은 SSD를 누르세요.'],
  ssd: ['SSD는 여러 칩이 함께 일하는 장치입니다. NAND Flash는 전원 없이 정보를 유지하는 반도체 메모리이고, Controller가 쓰고 읽는 일을 지휘합니다.', '밝은 테두리의 NAND를 누르거나 스크롤하여 안으로 들어가세요.'],
  package: ['기판에서 보이는 검은 칩은 보호용 패키지입니다. 그 안의 얇은 실리콘 조각인 Die에 실제 저장 소자와 회로가 만들어져 있습니다.', '먼저 덮개를 열고, 안에 드러난 Die를 눌러 보세요.'],
  die: ['Die에는 데이터를 담는 배열과 선택·읽기 회로가 함께 있습니다. Plane은 이 배열을 나눈 큰 구획입니다. 그림은 실제 칩 배치를 단순화한 개념도입니다.', '테두리로 표시한 Plane 0을 눌러 더 가까이 보세요.'],
  plane: ['많은 저장 공간을 작은 묶음으로 나누면 필요한 곳을 골라 동작시킬 수 있습니다. 그중 Block은 나중에 데이터를 지울 때의 기준이 됩니다.', '표시된 Block 1을 눌러 그 안의 데이터 단위를 보세요.'],
  block: ['파일 일부를 읽을 때 Block 전체를 읽을 필요는 없습니다. Page를 골라 읽고 쓰지만, 지울 때는 Block 전체가 영향을 받습니다. 층 모양은 동작 단위를 펼친 도식입니다.', '밝게 표시된 Page 3을 눌러 확대하세요.'],
  page: ['bit는 0 또는 1로 표현하는 정보의 단위입니다. Page는 함께 읽고 쓰는 bit의 묶음, Cell은 그 정보를 전기적 상태로 표현하는 소자입니다. 읽기 회로가 여러 Cell의 판정을 모읍니다.', '연결선을 따라 읽기 회로를 보고, 확대된 Cell을 누르세요.'],
  cell: ['절연 영역이 전하를 붙잡아 전원 없이도 상태가 남습니다. 이 모델에서는 저장 전자가 많을수록 Vth가 높아집니다. Cell 안에 0과 1이라는 숫자가 들어 있는 것은 아닙니다.', '전하 상태를 바꾸고 전원을 꺼 보세요. Vth의 차이가 남나요?'],
  density: ['1 bit에는 두 상태, 2 bit에는 네 상태가 필요합니다. 봉우리 하나는 비슷한 Vth를 가진 여러 Cell의 분포입니다. 한 Cell에 봉우리가 여러 개 있다는 뜻은 아닙니다.', 'SLC에서 QLC까지 선택하며 상태 수와 점선 사이 간격을 비교하세요.'],
  program: ['Program은 데이터를 쓰는 동작입니다. 펄스는 전압을 짧게 가하는 것이고, 그때마다 전하 상태를 조금씩 바꾼 뒤 목표에 도착했는지 확인합니다.', '펄스 버튼을 반복해서 누르세요. 막대가 목표 구간까지 이동합니다.'],
  read: ['Vref가 Vth보다 높으면 전류가 흐릅니다. 이 SLC 예시는 전류가 흐르면 1, 흐르지 않으면 0으로 판정합니다. 기준을 움직여도 저장 전하를 다시 쓰지는 않습니다.', '전압 슬라이더를 좌우로 움직여 전류와 0 / 1 판정이 바뀌는 지점을 찾으세요.'],
  erase: ['지워진 Cell에서 시작해야 원하는 값을 다시 기록할 수 있습니다. 하지만 Page 하나만 골라 지울 수는 없어, 주변 데이터까지 함께 지우는 문제가 생깁니다.', 'Page를 고르고 Program을 시도한 뒤, Block 전체 Erase와 비교하세요.'],
  address: ['FTL은 LBA와 PPA를 연결하는 주소 변환 계층입니다. 이 연결을 Mapping이라고 합니다. 그림의 B0:P0은 Block 0의 Page 0입니다.', 'LBA 100·101·102를 선택하며 연결된 물리 Page를 찾아보세요.'],
  ftl: ['Invalid Page에는 옛 데이터가 남아 있지만 더 이상 현재 데이터로 쓰지 않습니다. 새 Page에 기록한 뒤 주소 연결을 바꾸므로, 컴퓨터는 같은 LBA로 새 데이터를 찾습니다.', 'LBA 100을 두 번 수정해 보세요. 주소 연결과 옛 Page가 어떻게 바뀌나요?'],
  gc: ['Invalid는 빈 공간이 아닙니다. 옛 상태가 아직 남아 있습니다. Garbage Collection(GC)은 필요한 데이터부터 보존한 뒤 Block을 지워 공간을 되찾습니다.', 'GC를 실행하고 Valid 이동 → 주소 갱신 → 전체 지우기 순서를 보세요.'],
  wear: ['P/E Cycle은 Program(쓰기)과 Erase(지우기)를 한 차례 거치는 사용 단위입니다. Bad Block은 사용할 수 없어 할당에서 제외하는 Block입니다. 사용을 나눠도 이미 생긴 마모가 사라지지는 않습니다.', '분산을 끄고 실행한 뒤, 초기화하고 분산을 켜서 같은 6회를 비교하세요.'],
  reliability: ['Retention은 시간이 지나도 저장 상태를 유지하는 성질입니다. Read Disturb는 반복 읽기의 전압이 선택되지 않은 Cell의 상태에도 영향을 주는 현상입니다.', '원인을 고르고 슬라이더를 움직여 봉우리가 점선 경계를 넘는지 보세요.'],
  ecc: ['이 작은 SECDED 교육 모델은 1 bit 오류를 복구하고 2 bit 오류를 검출합니다. ECC는 검사 정보로 오류를 다루고, Retry는 저장 전하를 고치지 않고 읽기 기준을 바꿉니다.', '숫자 하나를 눌러 오류를 만든 뒤 ECC로 확인·복구하세요.'],
  write: ['Host는 저장을 요청하는 컴퓨터 쪽입니다. 파일의 변경은 주소와 데이터 요청으로 전달되고, Controller가 위치 선택·검사 정보·물리 기록을 연결합니다.', 'Write를 시작하고 다음 단계를 누르며 요청이 이동하는 곳을 따라가세요.'],
  'read-flow': ['FTL은 마지막으로 갱신한 주소를 찾습니다. ECC가 읽은 데이터를 확인하고, 필요하면 Read Retry가 다른 전압 기준으로 다시 판정합니다. 성공한 데이터가 문서로 돌아옵니다.', 'Read를 진행한 뒤 Retry 포함 경로도 비교해 보세요.'],
  ending: ['파일 하나의 기억에는 전하를 남기는 물리, 위치를 추적하는 주소표, 오류를 다루는 정보가 함께 필요합니다. 처음의 평범한 저장 버튼 뒤에서 일어난 일입니다.', '궁금한 주제는 아래 Article에서 이어 읽거나 처음부터 다시 탐험하세요.'],
};
...
export const articleSections = [
  ['computer','01. 저장한 문서는 왜 전원을 꺼도 남을까?',
    ...
    'SSD 안에서는 NAND Cell의 전하 상태가 데이터를 표현합니다. NAND의 비휘발성은 이 상태가 전원 없이도 유지된다는 성질입니다. 본편은 기록과 정상 종료를 마친 상황을 보여줍니다. 저장 버튼을 누른 순간 곧바로 모든 기록이 끝난다는 뜻은 아닙니다. 갑작스러운 전원 차단의 복구, 전원 손실 보호 기능, 파일 시스템의 일관성을 모두 재현하지는 않습니다.',
  ...
  ['structure','02. SSD, Package, Die, Plane, Block, Page',
    ...
    'Die 내부는 Plane 같은 구획으로 조직되고, 각 Plane에 여러 Block이 있습니다. Block은 함께 Erase하는 영역입니다. Read와 Program은 보통 Page를 기본 단위로 사용합니다. 본편에서 Block 안에 종이처럼 쌓아 보여준 Page는 동작 단위를 설명하는 도식입니다. 그 모양이 실제 반도체 내부의 층 모양과 일치한다는 뜻은 아닙니다. TLC에서는 같은 Cell 집합이 여러 논리 Page를 표현할 수 있습니다.',
  ...
  ['operations','06. Program, Read, Erase는 서로 다른 동작입니다',
    'Program은 전압을 이용해 저장 전하와 Vth를 목표 상태 쪽으로 이동시키는 과정입니다. 목표를 한 번에 정확히 맞추기 어려우므로 펄스를 가하고 결과를 확인하는 과정이 반복됩니다. 본편의 자동 반복에서는 펄스와 함께 막대가 목표 구간으로 이동합니다. 직접 살펴보기를 열면 펄스를 하나씩 가할 수도 있습니다. 실제 펄스 수, 전압과 검증 알고리즘을 모사하지는 않습니다.',
    'Read는 선택 Cell에 기준 전압을 가하고 전류 흐름을 감지합니다. 단순화한 SLC 예에서는 Vth보다 기준 전압이 높으면 도통하고 1로, 낮으면 비도통하고 0으로 판정합니다. 움직이는 기준선은 판정 기준이 달라지는 모습이며 저장 전하를 바꾸는 일이 아닙니다. 직접 살펴보기에서는 슬라이더로 같은 관계를 비교할 수 있습니다. 실제 NAND String에서는 다른 Cell을 통과시키기 위한 전압과 감지 회로도 필요합니다.',
  ...
  ['gc','08. Garbage Collection은 수정의 결과를 정리합니다',
    ...
    'GC는 먼저 대상 Block의 Valid 데이터를 다른 위치에 복사합니다. 다음으로 Mapping을 갱신해 새 복사본을 현재 데이터로 연결합니다. 남겨야 할 데이터가 모두 이동한 뒤 원래 Block을 Erase합니다. 그러면 한 Block 전체가 Free가 됩니다. 자동 설명의 GC는 FTL에서 LBA 100을 수정한 뒤의 상태에서 시작합니다. 각 반복은 같은 시작 상태로 돌아갑니다. 직접 살펴보기에서는 FTL과 GC가 저장 상태를 공유하므로 직접 수정한 결과가 GC의 출발점이 됩니다.',
  ...
  ['wear','09. 마모를 없애는 대신, 사용을 나눕니다',
    ...
    'Wear Leveling은 특정 Block에 사용이 몰리지 않도록 합니다. 본편은 같은 초기 상태에 같은 6회의 사용을 더해, 한곳에 집중하는 경우와 적게 사용한 Block을 우선 선택하는 경우를 나란히 보여줍니다. 막대 높이는 남은 수명이 아닌 사용 횟수입니다. Bad Block은 사용할 수 없는 Block이며 Controller가 할당 대상에서 제외합니다. 생산 때부터 존재할 수도 있고 사용 중에 생길 수도 있습니다.',
    'Dynamic Wear Leveling(동적 방식)은 새 쓰기에 사용할 Free Block을 고를 때 사용량을 고려합니다. Static Wear Leveling(정적 방식)은 오래 움직이지 않는 데이터가 들어 있는 Block도 관리에 포함해 편차를 줄입니다. 직접 살펴보기의 B0 제외 버튼은 불량 영역을 할당에서 제외하는 동작만 설명하며, 사용 횟수 하나로 실제 불량 여부를 판정한다는 뜻은 아닙니다. P/E 한계와 수명은 제품과 조건에 따라 달라집니다.',
  ...
  ['reliability','10. Retention과 Read Disturb: 상태의 경계는 고정되어 있지 않습니다',
    ...
    '본편에서는 Retention을 분포가 낮은 전압 쪽으로 이동하는 모습으로, Read Disturb를 높은 쪽으로 이동하는 모습으로 단순화했습니다. 실제 이동 방향과 크기, 분포 폭은 상태·구조·사용 이력·온도 등에 따라 달라집니다. 애니메이션의 진행 속도와 직접 조작의 슬라이더 값은 실제 시간이나 읽기 횟수를 환산한 것이 아닙니다.',
  ...
  ['ecc','11. ECC: 여분의 정보로 잘못 읽은 bit를 찾기',
    ...
    '본편은 확장 Hamming (8,4) SECDED를 실제 계산합니다. 데이터 4 bit에 검사 4 bit를 더해 총 8 bit를 만들고, 1 bit 오류는 위치를 찾아 수정하며, 2 bit 오류는 검출하되 수정하지 않습니다. 3 bit 이상에서는 이 보장이 성립하지 않으므로 직접 오류를 만드는 조작은 최대 2 bit로 제한합니다. 색상 외에 검사 결과 문장으로도 상태를 표시합니다. 실제 SSD의 ECC 전체 구현을 재현하는 모델은 아닙니다.',
...
// Summary, optional advanced paragraph index, and its visible disclosure label.
// Essential caveats remain in the core paragraphs; the source prose is not discarded.
export const articleReading = {
  computer: ['RAM은 작업 공간, SSD의 NAND는 전원 없이도 상태를 남기는 저장 매체입니다.', 1, '저장 완료와 영속화 · Flush / FUA'],
  structure: ['SSD 안의 NAND에서 Read / Program은 Page, Erase는 Block 단위로 이루어집니다.', 2, '논리 Page와 실제 Cell의 관계'],
  '3d': ['Cell을 위로 쌓는 것과 한 Cell에 여러 bit를 담는 것은 서로 다른 방법입니다.', 1, '수직 채널 · String · Wordline'],
  charge: ['Cell에 남긴 전하가 Vth를 바꾸고, 그 차이를 읽어 bit로 해석합니다.'],
  density: ['한 Cell에 더 많은 bit를 담을수록 더 촘촘한 상태를 구분해야 합니다.'],
  operations: ['Page는 읽고 쓰는 단위이고, Block은 함께 지우는 단위입니다.'],
  ftl: ['새 Page에 기록하고 주소 연결을 바꾸면, 같은 LBA로 수정한 데이터를 찾을 수 있습니다.', 2, '실제 Mapping과 메타데이터 전략'],
  gc: ['Invalid는 Free가 아닙니다. 필요한 데이터를 옮기고 Block을 지워야 다시 쓸 수 있습니다.', 2, '추가 쓰기 · Write Amplification'],
  wear: ['마모는 되돌릴 수 없으므로 사용을 나누고, 사용할 수 없는 Block은 제외합니다.', 2, 'Dynamic / Static Wear Leveling'],
  reliability: ['시간 경과나 반복 읽기로 Vth가 경계를 넘으면, 원래와 다른 bit로 읽힐 수 있습니다.', 1, '분포 변화 모델과 실제 조건'],
  ecc: ['ECC는 함께 저장한 검사 정보로 오류를 찾고, 정정 능력 안에서 복구합니다.', 2, '실제 SSD의 BCH / LDPC'],
  retry: ['Read Retry는 저장 전하를 고치지 않고, 기준 전압을 바꿔 다시 읽습니다.', 2, '추가 읽기 정보와 복구 한계'],
  flow: ['주소를 찾고, 물리 상태를 읽고 쓰고, 오류를 확인하는 일이 하나의 파일 요청으로 이어집니다.', 2, 'Controller의 병렬 처리와 일관성'],
};
export const renderArticle = () => articleSections.map(([id,title,a,b,c,sources,back]) => {
  const [summary,advanced,label]=articleReading[id],paragraphs=[a,b,c];
  const core=paragraphs.filter((_,i)=>i!==advanced).map(p=>`<p>${p}</p>`).join('');
  const detail=advanced===undefined?'':`<details class="article-depth"><summary><span>조금 더 정확히</span> ${label}</summary><p>${paragraphs[advanced]}</p></details>`;
  return `<section id="article-${id}" tabindex="-1" class="article-section"><h3>${title}</h3><p class="article-key"><span>핵심</span>${summary}</p>${core}${detail}<div class="article-sources">${sources.map(([label,url])=>`<a class="source" href="${url}" target="_blank" rel="noreferrer">${label} ↗</a>`).join('')}</div><button class="article-return" data-go="${back}">관련 장면에서 다시 살펴보기 ↑</button></section>`;
}).join('\n');
