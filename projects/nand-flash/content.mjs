...
export const sceneLearning = {
  ...
  cell: ['전하를 붙잡는 절연 영역이 기억을 유지합니다. Vth(문턱 전압)는 전류가 흐르기 시작하는 기준입니다. 이 모델에서는 저장 전자가 많을수록 Vth가 높아집니다.', '전하 상태를 바꾸고 전원을 꺼 보세요. Vth의 차이가 남나요?'],
  ...
  read: ['Vref는 읽을 때 가하는 기준 전압입니다. Vref가 Cell의 Vth보다 높아지면 전류가 흐릅니다. 이 SLC 예시는 그 차이를 1과 0으로 해석합니다.', '전압 슬라이더를 좌우로 움직여 전류와 0 / 1 판정이 바뀌는 지점을 찾으세요.'],
  ...
  address: ['LBA는 컴퓨터가 요청하는 주소, PPA는 NAND 안의 실제 위치입니다. FTL은 둘을 잇는 주소표를 관리합니다. 그림의 B0:P0은 Block 0의 Page 0입니다.', 'LBA 100·101·102를 선택하며 연결된 물리 Page를 찾아보세요.'],
  ftl: ['Valid는 현재 쓰는 데이터, Invalid는 쓸모없어진 옛 데이터, Free는 지워져서 쓸 수 있는 공간입니다. 주소 연결(Mapping)만 바꾸면 파일을 같은 주소에서 다시 열 수 있습니다.', 'LBA 100을 두 번 수정해 보세요. 주소 연결과 옛 Page가 어떻게 바뀌나요?'],
  ...
  wear: ['P/E Cycle은 쓰고 지우는 반복입니다. 반복 사용은 절연층과 읽기 특성을 바꿉니다. Wear Leveling은 마모를 분산하고, Bad Block은 사용할 수 없어 제외한 영역입니다.', '분산을 끄고 실행한 뒤, 초기화하고 분산을 켜서 같은 6회를 비교하세요.'],
  reliability: ['Retention은 저장 상태를 유지하는 성질입니다. 시간에 따른 전하 변화나 반복 읽기 전압의 영향(Read Disturb)으로 경계를 넘으면 원래와 다르게 읽힐 수 있습니다.', '원인을 고르고 슬라이더를 움직여 봉우리가 점선 경계를 넘는지 보세요.'],
  ecc: ['ECC는 오류를 고치기 위해 더해 두는 검사 정보입니다. 여기서는 데이터 4 bit에 검사 4 bit를 더해, 1 bit 오류는 복구하고 2 bit 오류는 찾아내는 작은 모델을 사용합니다.', '숫자 하나를 눌러 오류를 만든 뒤 ECC로 확인·복구하세요.'],
...
export const articleSections = [
  ...
  ['wear','09. 마모를 없애는 대신, 사용을 나눕니다',
    ...
    'Wear Leveling은 특정 Block에 사용이 몰리지 않도록 합니다. 동적 방식은 새 쓰기에 사용할 Free Block을 고를 때 사용량을 고려합니다. 정적 방식은 오래 움직이지 않는 데이터가 들어 있는 Block도 관리에 포함해 편차를 줄입니다. 본편은 같은 초기 상태에 같은 6회의 사용을 더해, 한곳에 집중하는 경우와 적게 사용한 Block을 우선 선택하는 경우를 나란히 보여줍니다. 막대 높이는 남은 수명이 아닌 사용 횟수입니다.',
    'Bad Block은 사용할 수 없는 Block입니다. 생산 때부터 존재할 수도 있고 사용 중에 생길 수도 있습니다. Controller는 이를 할당 대상에서 제외하고 나머지 공간을 관리합니다. 직접 살펴보기의 B0 제외 버튼은 이 동작만 설명하며, 사용 횟수 하나로 실제 불량 여부를 판정한다는 뜻은 아닙니다. P/E 한계와 수명은 제품과 조건에 따라 달라집니다.',
